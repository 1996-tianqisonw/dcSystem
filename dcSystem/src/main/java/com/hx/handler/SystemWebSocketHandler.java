...
public class SystemWebSocketHandler implements WebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        String id[] = url.split("=");//获取用户标志
        //users.add(session);
        // map.put(id[1],session);
        //table.put(id[1], session);
       // users.add(session);
        webSoketMap.put(id[1],session);//根据用户标志存储webcock
...
*/        System.out.println("关闭afterConnectionClosed" + closeStatus.getReason());
    ...
    public  String sendMessageToOne(String userId,String message){
       WebSocketMessage<String> message2 = new TextMessage(message);
       WebSocketSession  webSocketSession= webSoketMap.get(userId);
        try {
