...
<%--<form id="ff" method="post">
    <div>
        <label for="productid">编号:</label>
        <input id="productid" class="easyui-validatebox" type="text" name="bookname"
               data-options="validType:['length[6,10]']"/>
        <label for="productname">名称:</label>
        <input id="productname" class="easyui-validatebox" type="text" name="bookprice"
               data-options="validType:['length[6,10]']"/>
    ...
    $('#tt2').tree({
        ...
        onClick: function s(node) {
            if ($('#tt2').tree('isLeaf', node.target)) {
                ...
                $('#cid').val(node.id);
                /*查询对应菜单规格样式*/
/*                $.post("Categories/insertCategories.mvc", {cId: node.id}, function (data){
                    $('#dg').datagrid('loadData',data)
                })*/

                 $('#dg').datagrid({
                     url:'Specification/selectSpecification.mvc',
                     queryParams:{cId:node.id},
                     columns: [[
                         {checkbox: true},
                         {field: 'sId', title: 'id', width: 100,hidden: true},
                         {field: 'sName', title: '规格参数', width:100},
                         {field: 'sStyle', title: '样式选择', width: 100}
                     ]],
                     fitColumns: true,
                     toolbar: [{
                         iconCls: 'icon-add',
                         ...
                         handler: function () {
                             parent.$('#win_s').window({
                                 ...
                                 modal:true,
                                 collapsible:false,
                                 content:"<iframe src='specificationadd.jsp' height='100%' width='100%' frameborder='0px' ></iframe>"
                         });
                         ...
                         handler: function () {
                            ...
                            if (array.length==1){
                                parent.$('#win_s').window({
                                    ...
                                    modal:true,
                                    collapsible:false,
                                    content:"<iframe src='specificationupdata.jsp' height='100%' width='100%' frameborder='0px' ></iframe>"
                                });
                            }else {
                                $.messager.show({
                                    title:'提示信息',
                         ...
                         handler: function () {
                             ...
                             if(arr.length>0) {
                                 ...
                                 parent.$.messager.confirm('删除对话框', '您确认要删除吗？', function(r) {
                                     if (r) {
                                         $.ajax({
                                             url: "Specification/delSpecification.mvc",
                                             type:"POST",
                                             traditional:true,
                                             data:{ids:ids},
        ...
        onAfterEdit: function (node) {
            if (node.id == 0) {
                /*新增*/
                $.post("Categories/insertCategories.mvc", {cName: node.text}, function (data) {
                      if (data==1){
                          $.messager.show({
                              title:'提示信息',
                              msg:'添加成功。',
                          ...
                          //重新载人树
                          $.post("Categories/selectCategories.mvc", function (data) {
                ...
                /* 修改*/
                $.post("Categories/updataCategories.mvc", {cId: node.id, cName: node.text}, function (data) {
                    if (data == 1){
                        $.messager.show({
                            title:'提示信息',
                            msg:'操作成功。',
    ...
    function remove() {
        var selected = $('#tt2').tree('getSelected');
               $.post("Categories/delCategories.mvc", {cId: selected.id}, function (data) {
                   if (data == 1) {
